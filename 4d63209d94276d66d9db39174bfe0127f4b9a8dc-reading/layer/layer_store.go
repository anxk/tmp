...
// layerStore 表示了所有类型的层的 store，其中使用了多个 sync.Mutex 来尽可能的减小锁带来的性能下降
type layerStore struct {
	store       *fileMetadataStore
	driver      graphdriver.Driver
	useTarSplit bool

	layerMap map[ChainID]*roLayer // 保存在内存中的以 chainId 为键的 roLayer
	layerL   sync.Mutex

	mounts map[string]*mountedLayer  // 保存在内存中的以 containerId 为键的 mountedLayer
	mountL sync.Mutex
...
// Driver 返回 layerStore 对应的 graphdriver.Driver
func (ls *layerStore) Driver() graphdriver.Driver {
	return ls.driver
}

// loadLayer 加载层及其所有父层到内存中的 layerMap，这里返回值中的 *roLayer 最大用处就是为了函数中的递归调用，
// 注意层的 os 类型必须要与 layerStore 的 os 类型一致
func (ls *layerStore) loadLayer(layer ChainID) (*roLayer, error) {
	...
	// 校验层中的 os 类型是否与 layerStore 中的 os 类型一致
	if os != ls.os {
	...
	// 在 layerStore.layerMap 中保存层
	ls.layerMap[cl.chainID] = cl

	return cl, nil
}

// loadMount 加载 mountedLayer 到内存中的 mounts
